...
from core import LOG, Configuration
...
class AudioToTextRecorder:
    ...
    def __init__(
        ...
        self.porcupine_access_token = porcupine_access_token

        self.config = Configuration.get().get("listener").get("hotwords")[wake_words]

        self.is_shut_down = False
        self.shutdown_event = mp.Event()

        try:
            LOG.debug("Explicitly setting the multiprocessing start method to 'spawn'")
            mp.set_start_method("spawn")
        except RuntimeError as e:
            LOG.debug("Start method has already been set. Details:", e)

        LOG.info("Starting RealTimeSTT")
        ...
        if wake_words:
            ...
            keyword_file_paths = [
                expanduser(x.strip())
                for x in self.config.get("keyword_file_path", "").split(",")
            ]
            sensitivity_list = [
            ...
            try:
                self.porcupine = pvporcupine.create(
                    access_key=self.porcupine_access_token,
                ...
                LOG.exception(
                    "Error initializing porcupine " f"wake word detection engine: {e}"
                )
                raise
        ...
        LOG.debug("Waiting for main transcription model to start")
        self.main_transcription_ready_event.wait()
        LOG.debug("Main transcription model ready")

        LOG.debug("RealtimeSTT initialization completed successfully")
    ...
    def _transcription_worker(
        ...
        LOG.info(
            "Initializing faster_whisper " f"main transcription model {model_path}"
        )
        ...
        try:
            ...
            LOG.exception(
                "Error initializing main " f"faster_whisper transcription model: {e}"
            )
            raise
        ...
        LOG.debug(
            "Faster_whisper main speech to text "
            "transcription model initialized successfully"
        )
            ...
            try:
                if conn.poll(0.5):
                    ...
                    try:
                        ...
                        LOG.error(f"General transcription error: {e}")
    ...
    def _audio_data_worker(
        ...
        try:
            ...
            LOG.exception("Error initializing pyaudio " f"audio recording: {e}")
            raise

        LOG.debug("Audio recording (pyAudio input " "stream) initialized successfully")
        ...
        try:
            while not shutdown_event.is_set():
                try:
                    ...
                    if e.errno == pyaudio.paInputOverflowed:
                        LOG.warning("Input overflowed. Frame dropped.")
                    else:
            ...
            interrupt_stop_event.set()
            LOG.debug("Audio data worker process " "finished due to KeyboardInterrupt")
        finally:
            stream.stop_stream()
            stream.close()
    ...
    def start(self):
        ...
        if time.time() - self.recording_stop_time < self.min_gap_between_recordings:
            LOG.info("Attempted to start recording " "too soon after stopping.")
            return self
    ...
    def stop(self):
        ...
        if time.time() - self.recording_start_time < self.min_length_of_recording:
            LOG.info("Attempted to stop recording " "too soon after starting.")
            return self
    ...
    def set_microphone(self, microphone_on=True):
        ...
        """
        LOG.info("Setting microphone to: " + str(microphone_on))
    ...
    def shutdown(self):
        ...
        LOG.debug("Finishing recording thread")
        if self.recording_thread:
            self.recording_thread.join()

        LOG.debug("Terminating reader process")
        ...
        if self.reader_process.is_alive():
            LOG.warning(
                "Reader process did not terminate " "in time. Terminating forcefully."
            )
            self.reader_process.terminate()

        LOG.debug("Terminating transcription process")
        ...
        LOG.debug("Finishing realtime thread")
    ...
    def _recording_worker(self):
        ...
        LOG.debug("Starting recording worker")
        ...
        try:
                ...
                if not self.is_recording:
                    ...
                    if self.wake_words and wake_word_activation_delay_passed:
                        ...
                        if wakeword_index >= 0:
                            ...
                            start_index = max(
                                0, len(self.audio_buffer) - samples_for_0_1_sec
                            )
                    ...
                    if (
                        (not self.wake_words or not wake_word_activation_delay_passed)
                        and self.start_recording_on_voice_activity
                    ) or self.wakeword_detected:
                        if self._is_voice_active():
                            LOG.info("voice activity detected")
                    ...
                    if self.stop_recording_on_voice_deactivity:
                        ...
                        if (
                            ...
                            LOG.info("voice deactivity detected")
                            self.stop()
            ...
            if not self.interrupt_stop_event.is_set():
                LOG.error(f"Unhandled exeption in _recording_worker: {e}")
                raise
    ...
    def _realtime_worker(self):
        ...
        try:
            LOG.debug("Starting realtime worker")
            ...
            while self.is_running:
                ...
                if self.is_recording:
                    ...
                    if (
                        self.is_recording
                        and time.time() - self.recording_start_time > 0.5
                    ):
                        LOG.debug("Starting realtime transcription")
                            ...
                            if self.realtime_stabilized_safetext:
                                self._on_realtime_transcription_stabilized(
                                    self._preprocess_output(
                                        self.realtime_stabilized_safetext, True
                                    )
                                )
                            else:
                                self._on_realtime_transcription_stabilized(
            ...
            LOG.error(f"Unhandled exeption in _realtime_worker: {e}")
            raise
        ...
        for i in range(num_frames):
            ...
            if self.webrtc_vad_model.is_speech(frame, 16000):
                ...
                if not all_frames_must_be_true:
                    if self.debug_mode:
                        print(f"Speech detected in frame {i + 1}" f" of {num_frames}")
                    return True
        if all_frames_must_be_true:
            if self.debug_mode and speech_frames == num_frames:
                print(f"Speech detected in {speech_frames} of " f"{num_frames} frames")
